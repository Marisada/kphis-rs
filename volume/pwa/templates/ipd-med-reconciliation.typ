...
#if recons == none {recons = json(api + "ipd/med-reconcile?an=" + data.id)}
// PREPARED FUNCTIONS
...
#let render_rc(rc) = box[#v(5pt)
  ...
  #table(columns:(20pt,3fr,3fr,30pt,55pt,70pt,3fr,90pt,95pt),stroke:.5pt,
    table.header(table_h[\#],table_h[ชื่อยา],table_h[วิธีใช้],table_h[ได้รับ],table_h[วันที่ได้รับยา],table_h[สถานพยาบาล],table_h[คำสั่งแพทย์],table_h[Last Dose],table_h[จำนวนยาเหลือ/หมายเหตุ]),..rc.med_reconciliation_items.enumerate(start:1).map(tuple => item_row(tuple)).flatten())
]
// RENDER
