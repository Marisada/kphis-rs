#import "templates/utils.typ": api, get_patient_main, date_th, time_th, datetime_th, parse_dt, watermark
// PRELUDE
...
#let item(vs) = (cell(datetime_th(vs.vs_datetime)),
  ...
  cell(vs.lr_pos),cell(vs.lr_fsh),cell(vs.lr_cer),cell(vs.lr_int),cell(vs.lr_dur),cell(vs.lr_sta_name),cell(vs.lr_eff),cell(vs.lr_sev),cell(vs.lr_mem_name),cell(vs.lr_af),
  table.cell(align:horizon,text(size:10pt,[
    #if vs.had_name != none and vs.had_name.trim() != "" [#vs.had_name rate #vs.had_drop] #if vs.lr_oxytocin_unit != none [Oxytocin #vs.lr_oxytocin_unit U/L rate #vs.lr_oxytocin_rate dpm]
  ])),
...
#let cols = (90pt,22pt,22pt,18pt,35pt,22pt,24pt,22pt,22pt,18pt,28pt,22pt,15pt,22pt,20pt,22pt,20pt,1fr,120pt)
#let headers = (table_h[วันที่ เวลา],table_h[BT],table_h[PR],table_h[RR],table_h[BP],table_h[MAP],table_h[O#sub("2")sat],table_h[POS],table_h[FHS],table_h[Cx],table_h[INT],table_h[DUR],table_h[ST],table_h[EFF],table_h[SEV],table_h[MEM],table_h[AF],table_h[Medication],table_h[ผู้บันทึก])
#if has_o2 {
  cols.insert(7,80pt)
