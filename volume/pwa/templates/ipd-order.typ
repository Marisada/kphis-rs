...
#if doctor == none {doctor = json(api + "ipd/doctor-in-charge?an=" + data.id)}
#let oneday = data.at("oneday", default: none)
#if oneday == none {oneday = json(api + "ipd/order/order?an=" + data.id + "&order_type=oneday&view_by=doctor")}
#let cont = data.at("cont", default: none)
#if cont == none {cont = json(api + "ipd/order/order?an=" + data.id + "&order_type=continuous&view_by=doctor")}
...
#let vline(x) = place(top+left,line(stroke:.5pt,start:(x,120pt),end:(x,772pt)))
...
#set list(marker: [--])
#set page(paper:"a4",
  margin:(x:0.5cm,top:120pt,bottom:70pt),
  ...
  header:context[
    #grid(columns:(1fr,3fr,1fr),rows:2,
      ...
      grid.cell(rowspan:2,align:right,[FM-IPD-01/18 page #counter(page).display("1/1",both:true)]),
  ...
  footer:[#table(columns:(4fr,3fr,2fr,2fr,2fr),
    ...
    table.cell(colspan:3,label_note([Attending Physician],if doctor.len() > 0 {
      let main = doctor.filter(dr => dr.status == "Y")
