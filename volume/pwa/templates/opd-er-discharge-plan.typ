...
#if dc_plans == none {dc_plans = if pt.opd_er_order_master_id == none {()} else {json(api + "opd-er/dc-plan-id/" + str(pt.opd_er_order_master_id))}}
// PREPARED FUNCTIONS
...
#let render(dcp) = [
  ...
  #label_note([ชื่อ-สกุล : ],[#pt.pname #pt.fname #pt.lname]) #label_note([อายุ : ],[#pt.age_y ปี #pt.age_m เดือน])
    #label_note([HN : ],pt.hn) #label_note([VN : ],pt.vn)\
