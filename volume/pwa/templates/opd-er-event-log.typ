...
#if order == none {order = if pt.opd_er_order_master_id == none {()} else {json(api + "opd-er/order/order?opd_er_order_master_id=" + str(pt.opd_er_order_master_id) + "&view_by=doctor")}}
...
#if vs == none {vs = if pt.opd_er_order_master_id == none {()} else {json(api + "opd-er/vital-sign?opd_er_order_master_id=" + str(pt.opd_er_order_master_id))}}
#let io = data.at("io",default: none)
#if io == none {io = if pt.opd_er_order_master_id == none {()} else {json(api + "opd-er/io?opd_er_order_master_id=" + str(pt.opd_er_order_master_id))}}
  ...
  #badge(none,black,[NOTE])
    ...
    nt.progress_note_item_types.map(ts => {
      let items = if ts.progress_note_items.len() > 0 {
        ts.progress_note_items.map(item => item.progress_note_item_detail).join()
      } else []
...
#let all = (order_d + action_d + note_d + vs_d + io_d + lab_d).sorted(key:i => i.dt)
// RENDER
