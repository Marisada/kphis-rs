...
async fn test_med_reconcile_form_cpn_doctor() {
    let app = new_app();
    let cpn = kphis_ui_component::med_reconcile::med_reconcile_form::MedReconForm::new(
        Mutable::new(String::from("doctor")),
        Mutable::new(Some(date!(2023 - 12 - 31))),
        Mutable::new(kphis_model::tab::Tab::Order),
        Mutable::new(false),
        Mutable::new(false),
        Mutable::new(None),
        kphis_model::med_reconcile::MedReconciliation::demo(),
    );
...
async fn test_med_reconcile_main_cpn_doctor() {
    let app = new_app();
    let cpn = kphis_ui_component::med_reconcile::med_reconcile_main::MedReconcileCpn::new(
        Mutable::new(String::from("doctor")),
