...
// customized for backend note:
// - pass `ApiState` and `UserState` around via SystemWorld and ReadFn
...
impl SystemWorld {
    ...
    pub fn compile(&mut self, input: &str, data: &str, read_fn: ReadFn, app: Option<ApiState>, user: UserState) -> Result<PagedDocument, String> {
        self.reset();
        ...
        let watermark = ["โดย ", &user.user.name, "\n", &datetime_th(&now())].concat();
...
impl World for SystemWorld {
    ...
    fn source(&self, id: FileId) -> FileResult<Source> {
        self.slot(id, |slot| slot.source(self.read_fn, &self.app, &self.user))
    }

    fn file(&self, id: FileId) -> FileResult<Bytes> {
        self.slot(id, |slot| slot.file(self.read_fn, &self.app, &self.user))
    }
...
fn read_file(id: FileId, read_fn: ReadFn, app: &Option<ApiState>, user: &Option<UserState>) -> FileResult<Vec<u8>> {
...
impl FileSlot {
    ...
    fn source(&mut self, read_fn: Option<ReadFn>, app: &Option<ApiState>, user: &Option<UserState>) -> FileResult<Source> {
        self.source.get_or_init(read_fn, |reader| read_file(self.id, reader, app, user), |data, prev| create_source(self.id, &data, prev))
    }

    /// Retrieve the file's bytes.
    fn file(&mut self, read_fn: Option<ReadFn>, app: &Option<ApiState>, user: &Option<UserState>) -> FileResult<Bytes> {
        self.file.get_or_init(read_fn, |reader| read_file(self.id, reader, app, user), |data, _| Ok(Bytes::new(data)))
    }
}
