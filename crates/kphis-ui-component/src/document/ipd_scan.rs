...
/// - POST `EndPoint::IpdDocumentScanAn` (guarded, remove เพิ่มเอกสาร)
/// - DELETE `EndPoint::IpdDocumentScanAn` (guarded, remove delete image btn)
#[derive(Clone, Default)]
pub struct IpdDocumentScanCpn {
    an: Mutable<String>,
    is_editable: bool,
...
impl IpdDocumentScanCpn {
    pub fn new(an: &str, is_editable: bool) -> Rc<Self> {
        Rc::new(Self {
        ...
        if let Some(an) = str_some(page.an.get_cloned()) {
            app.async_load(
                ...
                clone!(app => async move {
                    // GET `EndPoint::IpdDocumentScanAn`
                    match DocumentScan::call_api_get_ipd(&an, app.state()).await {
                        Ok(responses) => {
    ...
    pub fn render(page: Rc<Self>, app: Rc<App>) -> Dom {
        ...
        let allow_post = app.endpoint_is_allow(&Method::POST, &EndPoint::IpdDocumentScanAn, is_pre_admit);
        let allow_delete = app.endpoint_is_allow(&Method::DELETE, &EndPoint::IpdDocumentScanAn, is_pre_admit);
        ...
        html!("div", {
                ...
                if !busy {
                    if let Some(add_type) = add_type_opt {
                        ...
                        app.loader_load(clone!(app, page => async move {
                            ...
                            match add_type.call_api_post_ipd(&page.an.get_cloned(), app.state()).await {
                                Ok(response) => {
                ...
                if !busy {
                    if let Some(delete_type) = delete_type_opt {
                        ...
                        app.loader_load(clone!(app, page => async move {
                            ...
                            match delete_type.call_api_delete_ipd(&page.an.get_cloned(), app.state()).await {
                                Ok(response) => {
                                    app.alert_execute_response(&response, async move {
                                        // app.alert("บันทึกข้อมูลสำเร็จ");
                                        page.loaded.set(false);
            ...
            .children([
                html!("div", {
                    ...
                    .children_signal_vec(page.result.signal_vec_cloned().filter_map(clone!(app, page => move |doc| {
                        (page.is_editable || doc.has_image).then(|| {
                            let image_box = ImageCpn::new_with_key(
                                ImageUsage::IpdDocument,
                                doc.document_id,
                                page.is_editable,
                                Mutable::new(None),
