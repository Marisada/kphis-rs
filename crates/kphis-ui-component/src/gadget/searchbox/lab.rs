...
use kphis_model::search::searchbox::LabSearchbox;
...
#[derive(Default)]
pub struct LabSearchboxCpn {
    text: Mutable<String>,
    results: MutableVec<Rc<LabSearchbox>>,
    status_text: Mutable<Option<String>>,
}

impl LabSearchboxCpn {
    pub fn new() -> Rc<Self> {
        Rc::new(Self {
        ...
        app.async_load(
            ...
            clone!(app, page => async move {
                ...
                if !search_text.is_empty() {
                    // GET `EndPoint::SearchBoxLabText`
                    match LabSearchbox::call_api_get(&search_text, app.state()).await {
                        Ok(results) => {
                                    ...
                                    .children_signal_vec(page.results.signal_vec_cloned().map(move |result| {
                                        html!("tr", {
                                            .child(html!("td", {
                                                ...
                                                .apply_if(result.component_type == Some(String::from("checkbox_group")), |dom| {
                                                    dom.class("text-success")
                                                })
                                                .text(&result.component_caption.clone().unwrap_or_default())
                                                .event(clone!(items, display_mutable, focus, changed, result => move |_: events::Click| {
                                                    if let Some(new) = result.component_caption.clone() {
                                                        ...
                                                        if let Some(item) = lock.last() {
                                                            ...
                                                            if old.is_empty() {
                                                                ...
                                                                item.order_item_detail.set([old.as_str(), &new].join(", "));
                                                            }
                                                            focus.set(Some(item.id));
                                                        } else {
                                                            let order_item = OrderItemMutable::new("lab", pre_order_master_id);
