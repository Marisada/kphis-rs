...
pub async fn get_ipd_dr_search_patient(
    ...
    let rows = match (
        ...
        (false, _, _, false, true, false, false) => query1_all(&doctor, &sql, pool, "Select IpdDrSearchPatient-18").await,
        (false, _, _, false, false, true, true) => query3_all(&consult, &consult, &passcode, &sql, pool, "Select IpdDrSearchPatient-19").await,
        (false, _, _, false, false, true, false) => query2_all(&consult, &consult, &sql, pool, "Select IpdDrSearchPatient-20").await,
        (false, _, _, false, false, false, true) => query1_all(&passcode, &sql, pool, "Select IpdDrSearchPatient-21").await,
        (false, _, _, false, false, false, false) => query_all(&sql, pool, "Select IpdDrSearchPatient-22").await,
    }?;

    rows.iter()
        .map(IpdSearchPatientDrResponse::from_row)
        .collect::<sqlx::Result<Vec<IpdSearchPatientDrResponse>>>()
        .map_err(|e| Source::SQLx.to_error(500, e, "Select IpdDrSearchPatient"))
}
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn sqlx_get_ipd_dr_search_patient() {
        ...
        // not discharged
        let default = get_ipd_dr_search_patient(IpdSearchPatientDrRequest::default(), 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        assert_eq!(default.len(), 13);

        // include discharged
        let found_hn = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {patient: Some(String::from("1234")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        assert_eq!(found_hn.len(), 2);
        let found_an = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {patient: Some(String::from("60001234")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        assert_eq!(found_an.len(), 1);
        let found_cid = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {patient: Some(String::from("1111111111111")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        assert_eq!(found_cid.len(), 2);
        let found_fullname = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {patient: Some(String::from("มมุติ")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        ...
        let found_consult = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {consult_dr_search: Some(String::from("008")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        assert_eq!(found_consult.len(), 1);

        // not discharged
        let no_passcode = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {passcode: Some(String::from("1234")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        ...
        let true_passcode = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {passcode: Some(String::from("1234")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
        assert_eq!(true_passcode.len(), 13);
        let false_passcode = get_ipd_dr_search_patient(IpdSearchPatientDrRequest {passcode: Some(String::from("6666")), ..Default::default()}, 7, 9, &tester.db_pool, &tester.hosxp, &tester.kphis).await.unwrap();
