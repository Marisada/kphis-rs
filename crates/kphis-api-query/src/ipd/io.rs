...
// GET /ipd/io-date/:an
pub async fn get_io_date(an: &str, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<IoDate>, AppError> {
...
pub async fn get_io_shift(
    ...
    let results = query
        .fetch_all(pool)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Select IpdIoShift"))?
        .iter()
...
fn from_row(row: &MySqlRow, shift_day_start: Time, shift_evening_start: Time, shift_night_start: Time) -> sqlx::Result<IoShift> {
    let io_date: Date = row.try_get("io_date")?;
    let io_time: Time = row.try_get("io_time")?;
    let (shift_date, shift) = NurseShift::generate(shift_day_start, shift_evening_start, shift_night_start, io_date, io_time);
    Ok(IoShift {
        io_id: row.try_get("io_id")?,
        io_date,
        io_time,
        io_parenteral_type: row.try_get("io_parenteral_type")?,
        io_parenteral_name: row.try_get("io_parenteral_name")?,
        io_parenteral_amount: row.try_get("io_parenteral_amount")?,
        io_parenteral_absorb: row.try_get("io_parenteral_absorb")?,
        io_parenteral_carry_forward: row.try_get("io_parenteral_carry_forward")?,
        io_parenteral_remark: row.try_get("io_parenteral_remark")?,
        io_oral_name: row.try_get("io_oral_name")?,
        io_oral_amount: row.try_get("io_oral_amount")?,
        io_oral_absorb: row.try_get("io_oral_absorb")?,
        io_oral_carry_forward: row.try_get("io_oral_carry_forward")?,
        io_oral_remark: row.try_get("io_oral_remark")?,
        io_output_type: row.try_get("io_output_type")?,
        io_output_amount: row.try_get("io_output_amount")?,
        io_output_remark: row.try_get("io_output_remark")?,
...
pub async fn get_io_only(an: &str, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<IoOnly>, AppError> {
    let sql = io::select_io_only(kphis);
    let results = sqlx::query(AssertSqlSafe(sql))
        .bind(an)
        .fetch_all(pool)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Select IpdIoOnly"))?
        .iter()
        .map(from_row_only)
...
fn from_row_only(row: &MySqlRow) -> sqlx::Result<IoOnly> {
    Ok(IoOnly {
        io_id: row.try_get("io_id")?,
        io_date: row.try_get("io_date")?,
        io_time: row.try_get("io_time")?,
        io_parenteral_type: row.try_get("io_parenteral_type")?,
        io_parenteral_name: row.try_get("io_parenteral_name")?,
        io_parenteral_amount: row.try_get("io_parenteral_amount")?,
        io_parenteral_absorb: row.try_get("io_parenteral_absorb")?,
...
async fn update_io_shift(io_id: u32, form: &IoShift, user: &str, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<Vec<ExecuteResponse>, AppError> {
    ...
    results.push(ExecuteResponse::from_query_result(update_result, "Update IpdIo"));
    // 2. Insert to history
    if is_update {
        let insert_history_io_result = insert_history_log(SourceTable::IpdIo, "U", user, &[KeyValue("io_id", io_id.to_string())], kphis, kphis_log, pool).await?;
...
async fn insert_io_shift(form: &IoShift, user: &str, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(2);
    // 1. Insert ipd_io
    ...
    results.push(ExecuteResponse::from_query_result(insert_io_result, "Insert IpdIo"));
    // 2. Insert to history
    if is_insert {
        let insert_history_io_result = insert_history_log(SourceTable::IpdIo, "I", user, &[KeyValue("io_id", id.to_string())], kphis, kphis_log, pool).await?;
        results.push(ExecuteResponse::from_query_result(insert_history_io_result, "Insert IpdIo History"));
    }
...
pub async fn insert_io_only_bundle(an: &str, only: &IoOnly, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(2);
    // 1. Insert ipd_io
    let insert_io_result = insert_io_only(an, only, pool, kphis).await?;
    let is_insert = insert_io_result.rows_affected() > 0;
    let id = insert_io_result.last_insert_id() as u32;
    results.push(ExecuteResponse::from_query_result(insert_io_result, "Insert IpdIoOnly"));
    // 2. Insert to history
    if is_insert {
        let insert_history_io_result = insert_history_log(SourceTable::IpdIo, "I", "system", &[KeyValue("io_id", id.to_string())], kphis, kphis_log, pool).await?;
        results.push(ExecuteResponse::from_query_result(insert_history_io_result, "Insert IpdIoOnly History"));
    }
...
async fn update_io(form: &IoShift, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    ...
    sqlx::query(AssertSqlSafe(update_io_sql))
        ...
        .execute(pool)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Update IpdIo"))
}
...
async fn insert_io(form: &IoShift, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    ...
    sqlx::query(AssertSqlSafe(insert_io_sql))
        ...
        .execute(pool)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Insert IpdIo"))
}

async fn insert_io_only(an: &str, only: &IoOnly, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
pub async fn delete_io_shift(io_id: u32, version: i32, user: &str, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<Vec<ExecuteResponse>, AppError> {
    ...
    // 2. Delete focus list
    if will_delete {
        let delete_io_result = delete_io(io_id, version, pool, kphis).await?;
        results.push(ExecuteResponse::from_query_result(delete_io_result, "Delete IpdIo"));
    }

    Ok(results)
}

async fn delete_io(io_id: u32, version: i32, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    ...
    sqlx::query(AssertSqlSafe(delete_io_sql))
        ...
        .execute(pool)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Delete IpdIo"))
}
...
mod tests {
    ...
    async fn sqlx_get_io_date() {
        ...
        let found = get_io_date("660001234", &tester.db_pool, &tester.kphis).await.unwrap();
        assert_eq!(found.len(), 3);
        let not_found = get_io_date("660006666", &tester.db_pool, &tester.kphis).await.unwrap();
    ...
    async fn sqlx_get_io_shift() {
        ...
        let find_an = get_io_shift(&IoParams {an: Some(String::from("660001234")),..Default::default()},time!(08:00),time!(16:00),time!(00:00),&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        ...
        let not_found = get_io_shift(&IoParams {an: Some(String::from("660006666")),..Default::default()},time!(08:00),time!(16:00),time!(00:00),&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_get_io_only() {
        ...
        let found = get_io_only("660001234",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(found.len(), 3);
        let not_found = get_io_only("660006666",&tester.db_pool,&tester.kphis).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_insert_io_only() {
        ...
        let success = insert_io_only("660001234", &IoOnly::demo(), &tester.db_pool, &tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_io_only("660001234", &IoOnly::demo(), &tester.db_pool, &tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
