...
pub async fn get_focus_list(an: &str, params: &FocusListParams, pool: &Pool<MySql>, hosxp: &str, kphis: &str) -> Result<Vec<FocusList>, AppError> {
...
pub async fn get_focus_list_only_bundle(an: &str, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<FocusListOnly>, AppError> {
    let mut focus_lists = get_focus_list_only(an, pool, kphis).await?;
...
async fn get_focus_list_only(an: &str, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<FocusListOnly>, AppError> {
...
pub async fn post_focus_list(an: &str, hn: &str, save: &FocusListSave, user: &str, pool: &Pool<MySql>, hosxp: &str, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    if let Some(fclist_id) = save.fclist_id {
        let results = update_focus_list_bundle(fclist_id, an, hn, save, user, pool, hosxp, kphis, kphis_log).await?;

        Ok((fclist_id, results))
    } else {
        insert_focus_list_bundle(an, hn, save, user, pool, kphis, kphis_log).await
    }
}
...
async fn update_focus_list_bundle(
    ...
    let update_focus_list_result = if used {
        update_focus_list_used(fclist_id, an, hn, save, user, pool, kphis).await?
    } else {
        update_focus_list_new(fclist_id, an, hn, save, user, pool, kphis).await?
    };
    ...
    if is_update {
        let insert_history_focus_list_result = insert_history_log(SourceTable::IpdFocusList, "U", user, &[KeyValue("fclist_id", fclist_id.to_string())], kphis, kphis_log, pool).await?;
    ...
    if !save.goal_ids.is_empty() {
        ...
        let insert_history_goal_result = insert_history_log(SourceTable::IpdFocusListGoalItem, "U", user, &[KeyValue("fclist_id", fclist_id.to_string())], kphis, kphis_log, pool).await?;
...
pub async fn insert_focus_list_bundle(an: &str, hn: &str, save: &FocusListSave, user: &str, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(4);
    // 1. Insert focus_list
    let insert_focus_list_result = insert_focus_list(an, hn, save, user, pool, kphis).await?;
    ...
    // 2. Insert to history_ipd_focus_list
    let insert_history_focus_list_result = insert_history_log(SourceTable::IpdFocusList, "I", user, &[KeyValue("fclist_id", fclist_id.to_string())], kphis, kphis_log, pool).await?;
    results.push(ExecuteResponse::from_query_result(insert_history_focus_list_result, "Insert FocusList History"));
    ...
    if !save.goal_ids.is_empty() {
        ...
        let insert_history_goal_result = insert_history_log(SourceTable::IpdFocusListGoalItem, "I", user, &[KeyValue("fclist_id", fclist_id.to_string())], kphis, kphis_log, pool).await?;
        results.push(ExecuteResponse::from_query_result(insert_history_goal_result, "Insert GoalItem History"));
    }
...
pub async fn insert_focus_list_only_bundle(an: &str, hn: &str, only: &FocusListOnly, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(4);
    // 1. Insert focus_list
    let insert_focus_list_result = insert_focus_list_only(an, hn, only, pool, kphis).await?;
    let fclist_id = insert_focus_list_result.last_insert_id() as u32;
    results.push(ExecuteResponse::from_query_result(insert_focus_list_result, "Insert FocusListOnly"));
    // 2. Insert to history_ipd_focus_list
    let insert_history_focus_list_result = insert_history_log(SourceTable::IpdFocusList, "I", "system", &[KeyValue("fclist_id", fclist_id.to_string())], kphis, kphis_log, pool).await?;
...
async fn update_focus_list_used(fclist_id: u32, an: &str, hn: &str, save: &FocusListSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn update_focus_list_new(fclist_id: u32, an: &str, hn: &str, save: &FocusListSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn insert_focus_list(an: &str, hn: &str, save: &FocusListSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn insert_focus_list_only(an: &str, hn: &str, only: &FocusListOnly, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
mod tests {
    ...
    async fn sqlx_get_focus_list() {
        ...
        let default = get_focus_list("660001234",&FocusListParams::default(),&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(default.len(), 2);
        let found_start_end = get_focus_list("660001234",&FocusListParams {start_date: date_8601("2024-01-01"),end_date: date_8601("2024-02-01"),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(found_start_end.len(), 2);
        let found_start_alone = get_focus_list("660001234",&FocusListParams {start_date: date_8601("2024-01-01"),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(found_start_alone.len(), 1);
        // will treat end_date as start_date
        let found_end_alone = get_focus_list("660001234",&FocusListParams {end_date: date_8601("2024-01-01"),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(found_end_alone.len(), 1);
        let found_status_fin = get_focus_list("660001234",&FocusListParams {status: Some(String::from("2")),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(found_status_fin.len(), 1);
        let found_status_unfin = get_focus_list("660001234",&FocusListParams {status: Some(String::from("1")),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(found_status_unfin.len(), 1);
        let found_id = get_focus_list("660001234",&FocusListParams {fclist_id: Some(1),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
        assert_eq!(found_id.len(), 1);
        let not_found = get_focus_list("660006666",&FocusListParams {fclist_id: Some(1),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_get_focus_list_only() {
        ...
        let found = get_focus_list_only("660001234", &tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(found.len(), 2);
        let not_found = get_focus_list_only("660006666",&tester.db_pool,&tester.kphis).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_insert_focus_list() {
        ...
        let success = insert_focus_list("660001234","0001234",&FocusListSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_focus_list("660001234","0001234",&FocusListSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_insert_focus_list_only() {
        ...
        let success = insert_focus_list_only("660001234","0001234",&FocusListOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_focus_list_only("660001234","0001234",&FocusListOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    async fn sqlx_update_focus_list_new() {
        ...
        let success = update_focus_list_new(1,"660001234","0001234",&FocusListSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_fail_version = update_focus_list_new(1,"660001234","0001234",&FocusListSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_update_focus_list_used() {
        ...
        let success = update_focus_list_used(1,"660001234","0001234",&FocusListSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_fail_version = update_focus_list_used(1,"660001234","0001234",&FocusListSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_fail_version.rows_affected(), 0);
    }
