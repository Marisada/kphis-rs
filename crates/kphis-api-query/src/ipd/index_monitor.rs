...
use kphis_sql::ipd::index_monitor;
...
async fn insert_index_monitor(save: &IndexMonitor, doctorcode: &str, user: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<MySqlQueryResult, AppError> {
    match &save.visit_type {
        ...
        VisitTypeId::OpdEr(_, _) | VisitTypeId::Visit(_) => Err(AppError::app_400("Insert IndexMonitor")),
    }
}
...
mod tests {
    ...
    async fn sqlx_insert_index_monitor() {
        ...
        let success = insert_index_monitor(&IndexMonitor::demo(), "008", "user",&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_index_monitor(&IndexMonitor::demo(), "008", "user",&tester.db_pool,&tester.kphis_extra).await.unwrap();
    ...
    async fn sqlx_insert_index_monitors_only() {
        ...
        let success = insert_index_monitors_only(1,"660001234",&[IndexMonitorOnly::demo()],&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_index_monitors_only(1,"",&[IndexMonitorOnly::demo()],&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
