...
use kphis_sql::{TABLE_CREATE_COLUMNS, TABLE_CREATE_PREPARED, TABLE_UPDATE_SET, ipd::vital_sign};
...
pub async fn get_vital_sign_only(an: &str, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<VitalSignOnly>, AppError> {
...
pub async fn post_vital_sign(an: &str, hn: &str, form: &VitalSignSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    form.insert(
        Some("vs_id"),
        Some("ipd_vs_vital_sign"),
        &[",an,hn", TABLE_CREATE_COLUMNS].concat(),
...
pub async fn insert_vital_sign_only(an: &str, hn: &str, only: &VitalSignOnly, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    only.insert(Some("vs_id"), Some("ipd_vs_vital_sign"), ",an,hn", ",?,?", &[an, hn], pool, kphis)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Insert VitalSignOnly"))
}

// ipd-vital-sign-save.php
pub async fn put_vital_sign(an: &str, hn: &str, form: &VitalSignSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    form.update("vs_id", Some("ipd_vs_vital_sign"), &[",an=?,hn=?", TABLE_UPDATE_SET].concat(), &[an, hn, user], pool, kphis)
        .await
...
mod tests {
    ...
    async fn sqlx_get_vital_sign() {
        ...
        let found_an = get_vital_sign(&VitalSignParams {an: Some(String::from("660001234")),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_get_vital_sign_only() {
        ...
        let found = get_vital_sign_only("660001234",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(found.len(), 3);
        let not_found = get_vital_sign_only("660006666",&tester.db_pool,&tester.kphis).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_post_vital_sign() {
        ...
        let success = post_vital_sign("660001234","0001234",&VitalSignSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = post_vital_sign("660001234","0001234",&VitalSignSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_insert_vital_sign_only() {
        ...
        let success = insert_vital_sign_only("660001234", "0001234", &VitalSignOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_vital_sign_only("660001234", "0001234", &VitalSignOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    async fn sqlx_put_vital_sign() {
        ...
        let success = put_vital_sign("660001234","0001234",&VitalSignSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = put_vital_sign("660001234","0001234",&VitalSignSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
