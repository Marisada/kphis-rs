...
use kphis_sql::{TABLE_UPDATE_SET, ipd::dc_plan};
use kphis_util::error::{AppError, Source};

pub async fn get_dc_plan(an: &str, pool: &Pool<MySql>, hosxp: &str, kphis_extra: &str) -> Result<Vec<DischargePlan>, AppError> {
...
pub async fn get_dc_plan_only_bundle(an: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<Vec<DischargePlanOnly>, AppError> {
    let mut dc_plans = get_dc_plan_only(an, pool, kphis_extra).await?;
...
async fn get_dc_plan_only(an: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<Vec<DischargePlanOnly>, AppError> {
...
pub async fn post_dc_plan(an: &str, save: &DischargePlanSave, user: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    if save.dc_plan_id > 0 {
        let results = update_dc_plan_bundle(save.dc_plan_id, an, save, user, pool, kphis_extra).await?;

        Ok((save.dc_plan_id, results))
    } else {
        insert_dc_plan_bundle(an, save, user, pool, kphis_extra).await
    }
}

pub async fn update_dc_plan_bundle(dc_plan_id: u32, an: &str, save: &DischargePlanSave, user: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<Vec<ExecuteResponse>, AppError> {
    let mut results = Vec::with_capacity(9);

    let update_dc_plan_result = update_dc_plan(an, save, user, pool, kphis_extra).await?;
...
pub async fn insert_dc_plan_bundle(an: &str, save: &DischargePlanSave, user: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(5);

    let insert_dc_plan_result = insert_dc_plan(an, save, user, pool, kphis_extra).await?;
...
pub async fn insert_dc_plan_only_bundle(an: &str, only: &DischargePlanOnly, pool: &Pool<MySql>, kphis_extra: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(5);

    let insert_dc_plan_result = insert_dc_plan_only(an, only, pool, kphis_extra).await?;
    let dc_plan_id = insert_dc_plan_result.last_insert_id() as u32;
...
pub async fn insert_dc_plan(an: &str, form: &DischargePlanSave, user: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<MySqlQueryResult, AppError> {
    form.insert(
        Some("dc_plan_id"),
        Some("ipd_dc_plan"),
        ",an,create_user,create_datetime,update_user,update_datetime,version",
        ",?,?,NOW(),?,NOW(),1",
        &[an, user, user],
        pool,
        kphis_extra,
    ...
    .map_err(|e| {
        ...
        {
            AppError::app_403_duplicate("Insert IpdDischargePlan")
        } else {
            Source::SQLx.to_error(500, e, "Insert IpdDischargePlan")
        }
    })
}

pub async fn insert_dc_plan_only(an: &str, only: &DischargePlanOnly, pool: &Pool<MySql>, kphis_extra: &str) -> Result<MySqlQueryResult, AppError> {
    only.insert(Some("dc_plan_id"), Some("ipd_dc_plan"), ",an", ",?", &[an], pool, kphis_extra).await.map_err(|e| {
        if let sqlx::error::Error::Database(err) = &e
            && err.code().map(|c| c == "23000").unwrap_or_default()
        {
            AppError::app_403_duplicate("Insert IpdDischargePlanOnly")
        } else {
            Source::SQLx.to_error(500, e, "Insert IpdDischargePlanOnly")
        }
    })
}

pub async fn update_dc_plan(an: &str, form: &DischargePlanSave, user: &str, pool: &Pool<MySql>, kphis_extra: &str) -> Result<MySqlQueryResult, AppError> {
    form.update("dc_plan_id", Some("ipd_dc_plan"), &[",an=?", TABLE_UPDATE_SET].concat(), &[an, user], pool, kphis_extra)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Update IpdDischargePlan"))
}
...
mod tests {
    ...
    async fn sqlx_get_dc_plan() {
        ...
        let found = get_dc_plan("660001234",&tester.db_pool,&tester.hosxp,&tester.kphis_extra).await.unwrap();
        assert_eq!(found.len(), 1);
        let not_found = get_dc_plan("660006666",&tester.db_pool,&tester.hosxp,&tester.kphis_extra).await.unwrap();
    ...
    async fn sqlx_get_dc_plan_only() {
        ...
        let found = get_dc_plan_only("660001234",&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(found.len(), 1);
        let not_found = get_dc_plan_only("660006666",&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_insert_dc_plan() {
        ...
        let success = insert_dc_plan("660001234", &DischargePlanSave::demo(),"user",&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        // duplicate (an + dx_id) will error
        let again_duplicate = insert_dc_plan("660001234", &DischargePlanSave::demo(),"user",&tester.db_pool,&tester.kphis_extra).await;
    ...
    async fn sqlx_insert_dc_plan_only() {
        ...
        let success = insert_dc_plan_only("660001234", &DischargePlanOnly::demo(),&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        // duplicate (an + dx_id) will error
        let again_duplicate = insert_dc_plan_only("660001234", &DischargePlanOnly::demo(),&tester.db_pool,&tester.kphis_extra).await;
        if let Err(again_duplicate_error) = again_duplicate {
            assert_eq!(again_duplicate_error.status, 403);
        } else {
            panic!("MUST ERROR")
    ...
    async fn sqlx_update_dc_plan() {
        ...
        let success = update_dc_plan("660001234", &DischargePlanSave::demo(),"user",&tester.db_pool,&tester.kphis_extra).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = update_dc_plan("660001234", &DischargePlanSave::demo(),"user",&tester.db_pool,&tester.kphis_extra).await.unwrap();
