...
use kphis_sql::opd_er::progress_note;
...
pub async fn get_progress_note_only_bundle(opd_er_order_master_id: u32, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<ProgressNoteOnly>, AppError> {
    let mut notes = get_progress_note_only(opd_er_order_master_id, pool, kphis).await?;
...
async fn get_progress_note_only(opd_er_order_master_id: u32, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<ProgressNoteOnly>, AppError> {
    ...
    match &save.visit_type {
        VisitTypeId::OpdEr(_, opd_er_order_master_id) => {
            ...
            if is_update {
                ...
                let update_result = update_progress_note_id(*opd_er_order_master_id, save, user, pool, kphis).await?;
                results.push(ExecuteResponse::from_query_result(update_result, "Update ProgressNoteSave"));

                let delete_item_result = delete_progress_note_item(save.progress_note_id.unwrap_or_default(), pool, kphis).await?;
                results.push(ExecuteResponse::from_query_result(delete_item_result, "Delete ProgressNoteItem"));
            } else {
                let insert_result = insert_progress_note(*opd_er_order_master_id, save, user, pool, kphis).await?;
                progress_note_id = Some(insert_result.last_insert_id() as u32);
                results.push(ExecuteResponse::from_query_result(insert_result, "Insert ProgressNote"));
            }
            if let Some(id) = progress_note_id {
                for progress_note_item in save.progress_note_items.iter() {
                    let insert_progress_note_item_result = insert_progress_note_item(*opd_er_order_master_id, id, progress_note_item, user, pool, kphis).await?;
                    results.push(ExecuteResponse::from_query_result(insert_progress_note_item_result, "Insert ProgressNoteItem"));
                }
            }

            Ok((progress_note_id.unwrap_or_default(), results))
        }
        VisitTypeId::Ipd(_) | VisitTypeId::PreAdmit(_) | VisitTypeId::Visit(_) => Err(AppError::app_400("Post ProgressNote")),
    }
}

async fn insert_progress_note(opd_er_order_master_id: u32, save: &ProgressNoteSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    let insert_sql = progress_note::insert_progress_note(save.progress_note_for_past_time.is_some(), kphis);
    let mut query = sqlx::query(AssertSqlSafe(insert_sql)).bind(opd_er_order_master_id);
...
pub async fn insert_progress_note_only(opd_er_order_master_id: u32, only: &ProgressNoteOnly, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn insert_progress_note_item(opd_er_order_master_id: u32, progress_note_id: u32, progress_note_item: &ProgressNoteItemSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
pub async fn insert_progress_note_items_only(progress_note_id: u32, opd_er_order_master_id: u32, progress_note_items: &[ProgressNoteItemOnly], pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    let insert_progress_note_item_sql = progress_note::insert_progress_note_items_only(progress_note_id, opd_er_order_master_id, progress_note_items, kphis);
    sqlx::query(AssertSqlSafe(insert_progress_note_item_sql))
        .execute(pool)
        .await
...
async fn update_progress_note_id(opd_er_order_master_id: u32, save: &ProgressNoteSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    let update_an_sql = progress_note::update_progress_note_id(save.progress_note_for_past_time.is_some(), kphis);
    let mut query = sqlx::query(AssertSqlSafe(update_an_sql)).bind(opd_er_order_master_id);
    if let Some(past_time) = save.progress_note_for_past_time {
        query = query.bind(past_time);
    }
...
mod tests {
    ...
    async fn sqlx_get_progress_note() {
        ...
        let found_an = get_progress_note(&ProgressNoteParams {opd_er_order_master_id: Some(1),..Default::default()},&[String::from("DOCTOR_INTERN")],&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
    ...
    async fn sqlx_get_progress_note_only() {
        ...
        let found = get_progress_note_only(1,&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(found.len(), 4);
        let not_found = get_progress_note_only(999,&tester.db_pool,&tester.kphis).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_get_progress_note_types() {
        ...
        sqlx::query(include_str!("../../../kphis-sqlx-tester/test_sqls/insert/kphis/opd_er_order_progress_note.sql")).execute(&tester.db_pool).await.unwrap();
        sqlx::query(include_str!("../../../kphis-sqlx-tester/test_sqls/insert/kphis/opd_er_order_progress_note_item.sql")).execute(&tester.db_pool).await.unwrap();
    ...
    async fn sqlx_insert_progress_note() {
        ...
        let success = insert_progress_note(1,&ProgressNoteSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_progress_note(1,&ProgressNoteSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
        save.progress_note_for_past_time = None;
        let again_time_now = insert_progress_note(1,&save,"user",&tester.db_pool,&tester.kphis).await.unwrap();
        ...
        let again_date_now = insert_progress_note(1,&save,"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_date_now.rows_affected(), 1);
        save.progress_note_for_past_time = None;
        let again_datetime_now = insert_progress_note(1,&save,"user",&tester.db_pool,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_insert_progress_note_only() {
        ...
        let success = insert_progress_note_only(1,&ProgressNoteOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_progress_note_only(1,&ProgressNoteOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    async fn sqlx_insert_progress_note_item() {
        ...
        let success = insert_progress_note_item(1,1,&ProgressNoteItemSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_progress_note_item(1,1,&ProgressNoteItemSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    async fn sqlx_insert_progress_note_items_only() {
        ...
        let success = insert_progress_note_items_only(1,1,&[ProgressNoteItemOnly::demo()],&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_progress_note_items_only(1,1,&[ProgressNoteItemOnly::demo()],&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    async fn sqlx_update_progress_note_id() {
        ...
        let mut save = ProgressNoteSave::demo();
        let success = update_progress_note_id(1, &save, "user", &tester.db_pool, &tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = update_progress_note_id(1, &save, "user", &tester.db_pool, &tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
        save.progress_note_for_past_time = None;
        let not_with_time_success = update_progress_note_id(1, &save, "user", &tester.db_pool, &tester.kphis).await.unwrap();
