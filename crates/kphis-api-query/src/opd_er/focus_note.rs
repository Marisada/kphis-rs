...
pub async fn get_focus_note(opd_er_order_master_id: u32, params: &FocusNoteParams, pool: &Pool<MySql>, hosxp: &str, kphis: &str, kphis_extra: &str) -> Result<Vec<FocusNote>, AppError> {
...
pub async fn get_focus_note_only_bundle(opd_er_order_master_id: u32, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<FocusNoteOnly>, AppError> {
    let mut focus_notes = get_focus_note_only(opd_er_order_master_id, pool, kphis).await?;
...
pub async fn get_focus_note_only(opd_er_order_master_id: u32, pool: &Pool<MySql>, kphis: &str) -> Result<Vec<FocusNoteOnly>, AppError> {
...
pub async fn post_focus_note(opd_er_order_master_id: u32, save: &FocusNoteSave, user: &str, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    if let Some(fcnote_id) = save.fcnote_id {
        let results = update_focus_note_bundle(fcnote_id, opd_er_order_master_id, save, user, pool, kphis, kphis_log).await?;

        Ok((fcnote_id, results))
    } else {
        insert_focus_note_bundle(opd_er_order_master_id, save, user, pool, kphis, kphis_log).await
    }
}
...
async fn update_focus_note_bundle(
    ...
    if !save.intvt_ids.is_empty() {
        ...
        let insert_history_intvt_result = insert_history_log(SourceTable::OpdErFocusNoteIntvtItem, "U", user, &[KeyValue("fcnote_id", fcnote_id.to_string())], kphis, kphis_log, pool).await?;
    ...
    if !save.dlc_ids.is_empty() {
        ...
        let insert_history_dlc_result = insert_history_log(SourceTable::OpdErFocusNoteDlcItem, "U", user, &[KeyValue("fcnote_id", fcnote_id.to_string())], kphis, kphis_log, pool).await?;
...
pub async fn insert_focus_note_bundle(
    ...
    // 2. Insert to history_ipd_focus_note
    let insert_history_focus_note_result = insert_history_log(SourceTable::OpdErFocusNote, "I", user, &[KeyValue("fcnote_id", fcnote_id.to_string())], kphis, kphis_log, pool).await?;
    results.push(ExecuteResponse::from_query_result(insert_history_focus_note_result, "Insert FocusNote History"));
    if !save.intvt_ids.is_empty() {
        ...
        let insert_history_intvt_result = insert_history_log(SourceTable::OpdErFocusNoteIntvtItem, "I", user, &[KeyValue("fcnote_id", fcnote_id.to_string())], kphis, kphis_log, pool).await?;
        results.push(ExecuteResponse::from_query_result(insert_history_intvt_result, "Insert FocusNoteIntvtItem History"));
    }
    if !save.dlc_ids.is_empty() {
        // 5. Insert dlcs item
        ...
        let insert_history_dlc_result = insert_history_log(SourceTable::OpdErFocusNoteDlcItem, "I", user, &[KeyValue("fcnote_id", fcnote_id.to_string())], kphis, kphis_log, pool).await?;
        results.push(ExecuteResponse::from_query_result(insert_history_dlc_result, "Insert FocusNoteDlcItem History"));
    }
...
pub async fn insert_focus_note_only_bundle(opd_er_order_master_id: u32, only: &FocusNoteOnly, pool: &Pool<MySql>, kphis: &str, kphis_log: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    let mut results = Vec::with_capacity(6);
    // 1. Insert focus_note
    let insert_focus_note_result = insert_focus_note_only(opd_er_order_master_id, only, pool, kphis).await?;
    let fcnote_id = insert_focus_note_result.last_insert_id() as u32;
    results.push(ExecuteResponse::from_query_result(insert_focus_note_result, "Insert FocusNoteOnly"));
    // 2. Insert to history_ipd_focus_note
    let insert_history_focus_note_result = insert_history_log(SourceTable::OpdErFocusNote, "I", "system", &[KeyValue("fcnote_id", fcnote_id.to_string())], kphis, kphis_log, pool).await?;
...
async fn update_focus_note(fcnote_id: u32, opd_er_order_master_id: u32, save: &FocusNoteSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn insert_focus_note(opd_er_order_master_id: u32, save: &FocusNoteSave, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn insert_focus_note_only(opd_er_order_master_id: u32, only: &FocusNoteOnly, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
...
async fn insert_intvt_items_only(fcnote_id: u32, intvts: &[FocusNoteIntvtItemOnly], pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    ...
    sqlx::query(AssertSqlSafe(insert_intvt_item_sql)).execute(pool).await.map_err(|e| {
        if let sqlx::error::Error::Database(err) = &e
            && err.code().map(|c| c == "23000").unwrap_or_default()
        {
            AppError::app_403_duplicate("Insert FocusNoteIntvtItemOnly")
        } else {
            Source::SQLx.to_error(500, e, "Insert FocusNoteIntvtItemOnly")
        }
    })
}
...
mod tests {
    ...
    async fn sqlx_get_focus_note() {
        ...
        let default = get_focus_note(1,&FocusNoteParams::default(),&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(default.len(), 3);
        let found_start_end = get_focus_note(1,&FocusNoteParams {start_date: date_8601("2024-01-01"),end_date: date_8601("2024-02-01"),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(found_start_end.len(), 2);
        let found_start_alone = get_focus_note(1,&FocusNoteParams {start_date: date_8601("2024-01-01"),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(found_start_alone.len(), 1);
        // will treat end_date as start_date
        let found_end_alone = get_focus_note(1,&FocusNoteParams {end_date: date_8601("2024-01-01"),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(found_end_alone.len(), 1);
        let found_list_id = get_focus_note(1,&FocusNoteParams {fclist_id: Some(1),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(found_list_id.len(), 2);
        let found_null_list_id = get_focus_note(1,&FocusNoteParams {fclist_id: Some(0),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(found_null_list_id.len(), 1);
        let found_note_id = get_focus_note(1,&FocusNoteParams {fcnote_id: Some(1),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
        assert_eq!(found_note_id.len(), 1);
        let not_found = get_focus_note(999,&FocusNoteParams {fcnote_id: Some(1),..Default::default()},&tester.db_pool,&tester.hosxp,&tester.kphis,&tester.kphis_extra).await.unwrap();
    ...
    async fn sqlx_get_focus_note_only() {
        ...
        let found = get_focus_note_only(1,&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(found.len(), 3);
        let not_found = get_focus_note_only(999,&tester.db_pool,&tester.kphis).await.unwrap();
        assert!(not_found.is_empty());
    }
    ...
    async fn sqlx_insert_focus_note() {
        ...
        let success = insert_focus_note(1,&FocusNoteSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_focus_note(1,&FocusNoteSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_insert_focus_note_only() {
        ...
        let success = insert_focus_note_only(1,&FocusNoteOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_focus_note_only(1,&FocusNoteOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    async fn sqlx_update_focus_note() {
        ...
        let success = update_focus_note(1,1,&FocusNoteSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_fail_version = update_focus_note(1,1,&FocusNoteSave::demo(),"user",&tester.db_pool,&tester.kphis).await.unwrap();
