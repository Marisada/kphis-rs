...
use kphis_sql::opd_er::index_action;
...
pub async fn post_index_action(save: &IndexAction, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<(u32, Vec<ExecuteResponse>), AppError> {
    ...
    if is_update {
        ...
        results.push(ExecuteResponse::from_query_result(update_action_result, "Update IndexAction"));
    } else {
        ...
        results.push(ExecuteResponse::from_query_result(insert_action_result, "Insert IndexAction"));
    }
...
async fn update_index_action(save: &IndexAction, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    ...
    sqlx::query(AssertSqlSafe(update_action_sql))
        ...
        .execute(pool)
        .await
        .map_err(|e| Source::SQLx.to_error(500, e, "Update IndexAction"))
}
...
async fn insert_index_action(save: &IndexAction, user: &str, pool: &Pool<MySql>, kphis: &str) -> Result<MySqlQueryResult, AppError> {
    if let VisitTypeId::OpdEr(_, opd_er_order_master_id) = &save.visit_type {
        ...
        sqlx::query(AssertSqlSafe(insert_action_sql))
            ...
            .execute(pool)
            .await
            .map_err(|e| Source::SQLx.to_error(500, e, "Insert IndexAction"))
    } else {
...
//     let insert_action_sql = index_action::insert_index_actions_only(plan_id, opd_er_order_master_id, index_actions_only, kphis);
...
mod tests {
    ...
    async fn sqlx_insert_index_action() {
        ...
        let mut index_action = IndexAction::demo();
        index_action.visit_type = VisitTypeId::OpdEr(String::new(), 1);

        let success = insert_index_action(&index_action,"user",&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_index_action(&index_action,"user",&tester.db_pool,&tester.kphis).await.unwrap();
    ...
    async fn sqlx_insert_index_action_only() {
        ...
        let success = insert_index_action_only(1,1,&mut IndexActionOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(success.rows_affected(), 1);
        let again_success = insert_index_action_only(1,1,&mut IndexActionOnly::demo(),&tester.db_pool,&tester.kphis).await.unwrap();
        assert_eq!(again_success.rows_affected(), 1);
    }
    ...
    //     let success = insert_index_actions_only(1,1,&[IndexActionOnly::demo()],&tester.db_pool,&tester.kphis).await.unwrap();
    //     assert_eq!(success.rows_affected(), 1);
    //     let again_success = insert_index_actions_only(1,1,&[IndexActionOnly::demo()],&tester.db_pool,&tester.kphis).await.unwrap();
    //     assert_eq!(again_success.rows_affected(), 1);
    // }
