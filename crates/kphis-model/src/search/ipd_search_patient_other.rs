...
pub struct IpdSearchPatientOtherRequest {
...
impl QueryString for IpdSearchPatientOtherRequest {
...
impl IpdSearchPatientOtherRequest {
    pub fn not_empty(self) -> Self {
        Self {
            ...
            passcode: self.passcode.as_ref().and_then(|s| str_some(s)),
        }
    }
}

/// IPD Search Patient result for Other user
#[derive(Demo, Deserialize, Serialize, FromRow, ToSchema)]
#[schema(example = json!(IpdSearchPatientOtherResponse::demo()))]
pub struct IpdSearchPatientOtherResponse {
...
impl IpdSearchPatientOtherResponse {
    /// GET `EndPoint::SearchOther`
    pub async fn call_api_get(request: &IpdSearchPatientOtherRequest, app: Rc<AppState>) -> Result<Vec<Self>, AppError> {
        match fetch_json_api(&[EndPoint::SearchOther.base(), request.clone().query_string()].concat(), "GET", None, app).await {
            Ok((response, true)) => {
                let response: Vec<Self> = serde_wasm_bindgen::from_value(response).map_err(|e| Source::SerdeWasm.to_teapot_error(e, "Fetch IpdSearchPatientOther"))?;
                Ok(response)
            }
            Ok((app_error, false)) => {
