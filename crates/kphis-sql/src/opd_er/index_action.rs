...
//    (SELECT EXISTS(SELECT * FROM kphis_extra.opd_er_nurse_index_monitor WHERE action_id=a.action_id)) AS has_monitor
// FROM kphis.opd_er_nurse_index_action a
//    LEFT JOIN kphis.opd_er_vs_vital_sign vs ON vs.action_id=a.action_id
...
// WHERE plan_id=? ORDER BY action_date,action_time;
/// plan_id
...
pub fn select_index_action_only(kphis: &str, kphis_extra: &str) -> String {
    [
        "SELECT a.*, (SELECT EXISTS(SELECT * FROM ",kphis_extra,".opd_er_nurse_index_monitor WHERE action_id=a.action_id)) AS has_monitor \
        FROM ",kphis,".opd_er_nurse_index_action a WHERE a.plan_id=? ORDER BY a.action_id;"
    ].concat()
}

// INSERT INTO kphis.opd_er_nurse_index_action (plan_id,opd_er_order_master_id,check_datetime,check_person,action_result,action_remark,action_date,action_time,action_report_back,action_blood_had,action_person_1,action_person_2,create_user,create_datetime,update_user,update_datetime,version)
// VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,NOW(),1);
/// plan_id, opd_er_order_master_id, check_datetime, check_person, action_result, action_remark, action_date, action_time, action_report_back, action_blood_had, action_person_1, action_person_2, loginname, loginname
pub fn insert_index_action(kphis: &str) -> String {
    [
        "INSERT INTO ",kphis,".opd_er_nurse_index_action (plan_id,opd_er_order_master_id,check_datetime,check_person,action_result,action_remark,action_date,action_time,action_report_back,action_blood_had,action_person_1,action_person_2",TABLE_CREATE_COLUMNS,") \
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?",TABLE_CREATE_PREPARED,");"
    ].concat()
}

pub fn insert_index_actions_only(plan_id: u32, opd_er_order_master_id: u32, index_actions_only: &[IndexActionOnly], kphis: &str) -> String {
...
// UPDATE kphis.opd_er_nurse_index_action
...
// "DELETE FROM ",kphis,".opd_er_nurse_index_action WHERE action_id=?;
/// action_id
