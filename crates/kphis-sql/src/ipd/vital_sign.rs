...
pub fn select_chart_data(params: &VitalSignParams, hosxp: &str, kphis: &str) -> String {
    ...
    let an = if params.an.is_some() {"  AND vs.an=? "} else {""};
    ...
    [
        ...
        WHERE 1=1 ",vs_id,an,start_date,end_date," ORDER BY vs.vs_datetime DESC;"
    ].concat()
}
...
pub fn select_vs_only(kphis: &str) -> String {
    [
        "SELECT * FROM ",kphis,".ipd_vs_vital_sign WHERE an=? ORDER BY vs_datetime DESC;"
    ].concat()
}

// DELETE FROM kphis.ipd_vs_vital_sign WHERE vs_id=?;
/// vs_id
pub fn delete_vital_sign(kphis: &str) -> String {
    ["DELETE FROM ", kphis, ".ipd_vs_vital_sign WHERE vs_id=?;"].concat()
}
