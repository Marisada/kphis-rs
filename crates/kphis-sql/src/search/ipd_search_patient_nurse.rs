use kphis_model::{score::CONCAT_SQL, search::ipd_search_patient_nurse::IpdSearchPatientNurseRequest};

#[derive(Debug, Default)]
pub struct FilterNurse {
...
impl super::FilterPatient for FilterNurse {
...
impl super::FilterWard for FilterNurse {
    fn has_ward(self) -> Self {
        Self { has_ward: true, ..self }
    }
}

impl super::FilterDoctor for FilterNurse {
    fn has_doctor(self) -> Self {
        Self { has_doctor: true, ..self }
    }
}

impl super::FilterPasscode for FilterNurse {
    fn has_passcode(self) -> Self {
        Self { has_passcode: true, ..self }
    }
}

pub fn sql_and_filter(params: IpdSearchPatientNurseRequest, hlen: usize, alen: usize, hosxp: &str, kphis: &str, kphis_extra: &str) -> (String, FilterNurse) {
    ...
    let (where_patient, filter) = match not_empty.patient.as_ref().and_then(|s| urlencoding::decode(s).ok()) {
        ...
        None => {
            ...
            let filter = FilterNurse::default();
...
// --  (SELECT CONCAT(vs.vs_datetime,',',kphis.score_total(kphis.score_bt(aa.age_y,vs.bt),kphis.score_pr(aa.age_y,vs.pr),
// --          kphis.score_rr(aa.age_y,vs.rr,vs.respirator),kphis.score_sbp(aa.age_y,vs.sbp,vs.inotrope),
// --          kphis.score_conscious_id(aa.age_y,vs.conscious_id),kphis.score_urine(aa.age_y,vs.urine_amount,vs.urine_duration)))
// --      FROM kphis.ipd_vs_vital_sign vs
// --      WHERE vs.an=ipt.an AND ((vs.bt IS NOT NULL AND TRIM(vs.bt) <> '')
// --          OR (vs.pr IS NOT NULL AND TRIM(vs.pr) <> '')
// --          OR ((vs.rr IS NOT NULL AND TRIM(vs.rr) <> '') AND (vs.respirator IS NOT NULL AND TRIM(vs.respirator) <> ''))
// --          OR ((vs.sbp IS NOT NULL AND TRIM(vs.sbp) <> '') AND (vs.inotrope IS NOT NULL AND TRIM(vs.inotrope) <> ''))
// --          OR (vs.conscious_id IS NOT NULL AND TRIM(vs.conscious_id) <> '')
// --          OR ((vs.urine_amount IS NOT NULL AND TRIM(vs.urine_amount) <> '') AND (vs.urine_duration IS NOT NULL AND TRIM(vs.urine_duration) <> '')))
// --      ORDER BY vs.vs_datetime DESC LIMIT 1) AS vs_datetime_mews,
//     (SELECT CONCAT(IFNULL(vs_datetime,''),'|',IFNULL(bt,''),'|',IFNULL(pr,''),'|',IFNULL(rr,''),'|',IFNULL(sbp,''),'|',IFNULL(inotrope,''),'|',IFNULL(respirator,''),'|',IFNULL(conscious_id,''),'|',
//         IFNULL(urine_amount,''),'|',IFNULL(urine_duration,''),'|',IFNULL(sat,''),'|',IFNULL(o2_id,''),'|',IFNULL(breathing_id,''),'|',IFNULL(avpu_id,''),'|',IFNULL(gut_feeling_id,''),'|',
//         IFNULL(pops_other_id,''),'|',IFNULL(wbc,''),'|',IFNULL(eye,''),'|',IFNULL(verbal,''),'|',IFNULL(movement,''),'|',IFNULL(crt,''),'|',IFNULL(band,'')) FROM kphis.ipd_vs_vital_sign
//         WHERE an=ipt.an GROUP BY vs_datetime ASC LIMIT 1) AS ews_concat,
...
//     (SELECT MAX(CONCAT(order_date,' ',order_time)) FROM kphis.ipd_order WHERE ipd_order.an=ipt.an AND ipd_order.order_confirm='Y') AS max_order_datetime,
//     IF(wp.ward IS NOT NULL,1,0) AS wp_status,
//     (SELECT EXISTS(SELECT * FROM kphis.ipd_dr_admission_note dan WHERE dan.an=ipt.an)) AS dr_admission_note_exists,
//     (SELECT EXISTS(SELECT * FROM kphis.ipd_nurse_admission_note nan WHERE nan.an=ipt.an)) AS nurse_admission_note_exists,
//     (SELECT EXISTS(SELECT * FROM kphis.ipd_order WHERE ipd_order.an=ipt.an AND ipd_order.order_confirm='Y' AND ipd_order.order_owner_type='doctor' AND ipd_order.nurse_accept_time IS NULL)) AS nurse_not_accept_exists,
...
//     (SELECT EXISTS(SELECT * FROM kphis.ipd_order
//         JOIN kphis.ipd_order_item ON ipd_order.order_id=ipd_order_item.order_id AND ipd_order_item.stat='Y'
//         WHERE ipd_order.an=ipt.an AND ipd_order.nurse_accept_time IS NULL AND ipd_order.order_confirm='Y' AND ipd_order.order_owner_type='doctor')) AS stat_order_nurse_not_accept_exists,
...
// [ MONITOR DRAFT ]
//
// SELECT ia.action_id,dud.monitor_status,dud.monitor_count,dud.monitor_duration,COUNT(im.monitor_id) AS m_count,TIMESTAMPDIFF(MINUTE,TIMESTAMP(ia.action_date, ia.action_time),MAX(im.monitor_datetime)) AS m_duration,
//   (SELECT monitor_abnormal FROM kphis_extra.ipd_nurse_index_monitor WHERE action_id=ia.action_id AND monitor_abnormal IS NOT NULL AND monitor_datetime IS NOT NULL ORDER BY monitor_datetime DESC LIMIT 1) AS monitor_abnormal
// FROM kphis.ipd_nurse_index_action ia
//   LEFT JOIN kphis_extra.ipd_nurse_index_monitor im ON im.action_id=ia.action_id AND im.monitor_abnormal IS NOT NULL AND im.monitor_datetime IS NOT NULL
//   LEFT JOIN kphis.ipd_nurse_index_plan ip ON ip.plan_id=ia.plan_id
//   LEFT JOIN kphis.ipd_order_item oi ON oi.order_item_id=ip.order_item_id
//   LEFT JOIN kphis.kphis_drug_use_duration dud ON dud.icode=oi.icode
// WHERE ia.an='660001509' AND ia.action_date IS NOT NULL AND ia.action_time IS NOT NULL
// GROUP BY ia.action_id
//   HAVING monitor_abnormal='Y' OR (monitor_status='Y' AND ((monitor_count > 0 AND m_count=0) OR
//    (monitor_abnormal='N' AND (monitor_count > m_count OR monitor_duration IS NULL OR monitor_duration > m_duration))));